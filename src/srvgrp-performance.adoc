...
operates on an metric-less integer performance scale. The individual point of
performance scale is also refer as `Performance Level` and represents a
performance operating point. What this scale represents and the metric is
entirely platform dependent. Values on this scale are discrete, and the platform
has complete control over mapping these performance operating points to
performance states, which are eventually converted into hardware parameters such
as voltage and frequency. For example, the mapping between levels and
frequencies can be as straightforward as using a multiplication factor of `1000`.
...
The following table shows the structure of a single performance level attribute.
...
Refer to <<section-perf-level-attribute>> for the structure of performance
level attribute.
